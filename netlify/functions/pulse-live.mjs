export default async function(req){
  ...
  const markets=[];const seenMarket=new Set();let articles=[];let podcast=[];
  const pushMarket=m=>{if(m&&!seenMarket.has(m.id)){seenMarket.add(m.id);markets.push(m)}};
  const mapPoly=m=>{
    let o=[],p=[];try{o=JSON.parse(m.outcomes||"[]");p=JSON.parse(m.outcomePrices||"[]")}catch{return null}
    const yi=o.findIndex(x=>String(x).toLowerCase()==="yes");if(yi<0)return null;
    const yes=Math.max(0,Math.min(100,Math.round(Number(p[yi]||0)*100)));
    return {id:"poly-"+m.id,source:"POLYMARKET",title:m.question,yes,no:100-yes,volume24h:Number(m.volume24hr||0),change24h:Number(m.oneDayPriceChange||0)*100,endDate:m.endDate||m.endDateIso||"",image:m.image||m.icon||""};
  };
  const tagSlugs=["pop-culture","music","movies"];
  const jobs=await Promise.allSettled([
    ...
    ...tagSlugs.map(slug=>json("https://gamma-api.polymarket.com/events?tag_slug="+slug+"&closed=false&limit=40&order=volume24hr&ascending=false"))
  ]);
  ...
  for(let i=4;i<jobs.length;i++){
    if(jobs[i].status!=="fulfilled")continue;
    for(const event of jobs[i].value||[]){
      const eventText=(event.title||"")+" "+(event.slug||"");
      if(blocked.some(x=>eventText.toLowerCase().includes(x)))continue;
      for(const m of event.markets||[]){
        const text=(m.question||"")+" "+eventText;
        if(blocked.some(x=>text.toLowerCase().includes(x)))continue;
        pushMarket(mapPoly(m));
        if(markets.length>=18)break;
      }
      if(markets.length>=18)break;
    }
  }
  ...
  if(jobs[1].status==="fulfilled"){
    for(const m of jobs[1].value||[]){
      if(!ok((m.question||"")+" "+(m.events?.[0]?.title||"")))continue;
      pushMarket(mapPoly(m));
      if(markets.length>=22)break;
    }
  }

  if(jobs[2].status==="fulfilled"){
    for(const m of jobs[2].value.markets||[]){
      if(!ok((m.title||"")+" "+(m.yes_sub_title||"")))continue;
      const p=Number(m.last_price_dollars||m.yes_ask_dollars||m.yes_bid_dollars||0);if(!(p>0&&p<1))continue;
      const yes=Math.round(p*100);
      const mapped={id:"kalshi-"+m.ticker,source:"KALSHI",title:m.title,yes,no:100-yes,volume24h:Number(m.volume_24h_fp||0),change24h:(p-Number(m.previous_price_dollars||p))*100,endDate:m.close_time||m.expiration_time||"",image:""};
      pushMarket(mapped);
      if(markets.length>=26)break;
    }
  }
